      ...
      const tl = gsap.timeline({
        scrollTrigger: {
          ...
          onUpdate: (self) => {
            ...
            // Stage 1: Video 1 (0.1 - 0.35)
            if (progress >= 0.1 && progress < 0.35) {
              const vid1 = video1Ref.current;
              if (vid1 && vid1.duration) {
                const stageProgress = (progress - 0.1) / (0.35 - 0.1); // 0 to 1 within this stage
                vid1.currentTime = stageProgress * vid1.duration;
              }
            }
           
            // Stage 2: Video 2 (0.35 - 0.60)
            else if (progress >= 0.35 && progress < 0.60) {
              const vid2 = video2Ref.current;
              if (vid2 && vid2.duration) {
                const stageProgress = (progress - 0.35) / (0.60 - 0.35);
                vid2.currentTime = stageProgress * vid2.duration;
              }
            }
           
            // Stage 3: Video 3 (0.60 - 1.0)
            else if (progress >= 0.60) {
              const vid3 = video3Ref.current;
              if (vid3 && vid3.duration) {
                const stageProgress = (progress - 0.60) / (1.0 - 0.60);
                vid3.currentTime = stageProgress * vid3.duration;
              }
            }
          },
        },
    ...
    <div id="why-revive" ref={containerRef} className="relative w-full h-[150vh] bg-black overflow-hidden z-10">
      <div className="sticky top-0 w-full h-screen flex flex-col items-center justify-start pt-[15vh]">
        ...
        <div className="relative flex-1 w-full max-w-6xl mt-4 pointer-events-none z-10">
          ...
          {/* Layer 2: Problem */}
          <div ref={layer2Ref} className="absolute inset-0 flex flex-col items-center text-center opacity-0">
